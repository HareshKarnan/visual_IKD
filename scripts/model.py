...
class VisualIKDNet(nn.Module):
    def __init__(self, input_size, output_size, hidden_size=32):
        ...
        self.imu_net = nn.Sequential(
            nn.Linear(200 * 3, 128), nn.BatchNorm1d(128), nn.PReLU(),
        ...
        self.ikdmodel = nn.Sequential(
            nn.Linear(input_size - (200 * 3) + 16 + 16, hidden_size), nn.BatchNorm1d(hidden_size), nn.PReLU(),
    ...
    def forward(self, odom_1sec_history, odom, image):
        visual_embedding = self.visual_encoder(image)
        imu_embedding = self.imu_net(odom_1sec_history)
...
class SimpleIKDNet(nn.Module):
    def __init__(self, input_size, output_size, hidden_size=32):
        ...
        self.ikdmodel = nn.Sequential(
            nn.Linear(input_size - (200*3) + 16, hidden_size), nn.BatchNorm1d(hidden_size), nn.PReLU(),
            nn.Linear(hidden_size, hidden_size), nn.BatchNorm1d(hidden_size), nn.PReLU(),
            nn.Linear(hidden_size, hidden_size), nn.BatchNorm1d(hidden_size), nn.PReLU(),
            nn.Linear(hidden_size, output_size),
        )

    def forward(self, odom_1sec_history, odom):
        imu_embedding = self.imu_net(odom_1sec_history)
