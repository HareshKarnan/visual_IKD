...
if __name__ == '__main__':
    ...
    times = np.linspace(0, total_time, args.frequency * total_time + 1)
    ...
    #     x_vel, y_vel, w_vel = filtered_data['odom'][i]
    #
    ...
    axs[0].plot(time_axes, filtered_data['odom'][:, 0],'-b' ,label='odom_x')
    axs[0].plot(time_axes, filtered_data['joystick'][:, 0], '--b',label='joy_x')
    axs[0].legend()
    # plt.plot(time_axes, filtered_data['odom'][:, 1], label='odom_y')
    axs[1].plot(time_axes, filtered_data['odom'][:, 2], '-r', label='odom_w')
    axs[1].plot(time_axes, filtered_data['joystick'][:, 1], '--r',label='joy_w')
    axs[1].legend()
    plt.show()
